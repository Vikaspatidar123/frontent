...
const fakeBackend = () => {
	...
	mock.onGet(new RegExp(`${url.GET_PRODUCTS_DETAIL}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (productsData) {
						...
						const product = productsData.find(
							(prd) => prd.id.toString() === params.id.toString()
						);
	...
	mock.onPost(url.GET_MAILS_LIST).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (mailDB.allmail) {
	...
	mock.onPost(url.SET_FOLDER_SELECTED_MAILS).reply(
		(request) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPut(url.UPDATE_MAIL).reply(
		(mail) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (mail && mail.data) {
						...
						mailDB.allmail = mailDB.allmail.map((ml) => {
							if (ml.id === data.id) {
								return { ...ml, ...data };
							}
							return mail;
	...
	mock.onPost(url.ADD_NEW_USER).reply(
		(user) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (user && user.data) {
	...
	mock.onPut(url.UPDATE_USER).reply(
		(user) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (user && user.data) {
	...
	mock.onDelete(url.DELETE_USER).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPost(url.ADD_NEW_PROJECT).reply(
		(project) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPut(url.UPDATE_PROJECT).reply(
		(project) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (project && project.data) {
	...
	mock.onDelete(url.DELETE_PROJECT).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onPost(url.ADD_NEW_EVENT).reply(
		(event) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (event && event.data) {
	...
	mock.onPut(url.UPDATE_EVENT).reply(
		(event) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (event && event.data) {
	...
	mock.onDelete(url.DELETE_EVENT).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onGet(new RegExp(`${url.GET_MESSAGES}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (messages) {
	...
	mock.onPost(url.ADD_MESSAGE).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config.data) {
	...
	mock.onDelete(url.DELETE_MESSAGE).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onPost(url.ADD_NEW_ORDER).reply(
		(order) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (order && order.data) {
	...
	mock.onPut(url.UPDATE_ORDER).reply(
		(order) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (order && order.data) {
	...
	mock.onDelete(url.DELETE_ORDER).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onPost(url.ADD_NEW_CUSTOMER).reply(
		(customer) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPut(url.UPDATE_CUSTOMER).reply(
		(customer) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (customer && customer.data) {
	...
	mock.onDelete(url.DELETE_CUSTOMER).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onGet(new RegExp(`${url.GET_INVOICE_DETAIL}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (invoiceList) {
						...
						const invoice = invoiceList.find(
							(invce) => invce.id.toString() === params.id.toString()
						);
						resolve([200, invoice]);
					} else {
	...
	mock.onPost(url.ADD_NEW_JOB_LIST).reply(
		(jobList) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPut(url.UPDATE_JOB_LIST).reply(
		(jobList) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (jobList && jobList.data) {
	...
	mock.onDelete(url.DELETE_JOB_LIST).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onDelete(url.DELETE_APPLY_JOB).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onGet(new RegExp(`${url.GET_PROJECT_DETAIL}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (projects) {
						...
						const project = projects.find(
							(prj) => prj.id.toString() === params.id.toString()
						);
						resolve([200, project]);
					} else {
	...
	mock.onPost(url.ADD_CARD_DATA).reply(
		(kanbanData) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
	...
	mock.onPut(url.UPDATE_CARD_DATA).reply(
		(kanbanData) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (kanbanData && kanbanData.data) {
	...
	mock.onDelete(url.DELETE_KANBAN).reply(
		(config) =>
			new Promise((resolve, reject) => {
				setTimeout(() => {
					if (config && config.headers) {
	...
	mock.onGet(new RegExp(`${url.TOP_SELLING_DATA}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { params } = config;
				setTimeout(() => {
	...
	mock.onGet(new RegExp(`${url.GET_EARNING_DATA}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { params } = config;
				setTimeout(() => {
					if (params && params.month) {
						// Passing fake JSON data as response
						const { params: prms } = config;
						let data = [];
						if (prms.month === 'jan') {
							data = janEarningData;
						} else if (prms.month === 'dec') {
							data = decEarningData;
						} else if (prms.month === 'nov') {
							data = novEarningData;
						} else if (prms.month === 'oct') {
							data = octEarningData;
						}
						resolve([200, data]);
					} else {
	...
	mock.onGet(new RegExp(`${url.GET_VISITOR_DATA}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { params } = config;
				setTimeout(() => {
					if (params && params.duration) {
						// Passing fake JSON data as response
						const { params: prm } = config;
						let data = [];

						if (prm.duration === 'onemonth') {
							data = visitor.onemonth;
						} else if (prm.duration === 'sixmonth') {
							data = visitor.sixmonth;
						} else if (prm.duration === 'year') {
							data = visitor.year;
						} else {
							data = visitor.all;
						}
	...
	mock.onGet(new RegExp(`${url.GET_STATISTICS_DATA}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { params } = config;
				setTimeout(() => {
					if (params && params.duration) {
						...
						if (prm.duration === 'month') {
							data = statisticData.month;
						} else if (prm.duration === 'week') {
							data = statisticData.week;
						} else {
							data = statisticData.year;
						}
						resolve([200, data]);
					} else {
	...
	mock.onGet(new RegExp(`${url.GET_WALLET_DATA}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { params } = config;
				setTimeout(() => {
					if (params && params.month) {
						...
						if (prm.month === 'jan') {
							data = janWalletData;
						} else if (prm.month === 'march') {
							data = marchWalletData;
						} else if (prm.month === 'feb') {
							data = febWalletData;
						} else if (prm.month === 'dec') {
							data = decWalletData;
						}
						resolve([200, data]);
					} else {
	...
	mock.onPost(new RegExp(`${url.ON_LIKNE_COMMENT}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { data } = config;
				...
				setTimeout(() => {
					if (productComments) {
						const modifiedProductComments = [...productComments];
						...
						if (commentIdx > -1) {
							...
							} else if (modifiedProductComments[commentIdx].hasLiked) {
	...
	mock.onPost(new RegExp(`${url.ON_ADD_REPLY}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { data } = config;
				const { params } = JSON.parse(data);
	...
	mock.onPost(new RegExp(`${url.ON_ADD_COMMENT}/*`)).reply(
		(config) =>
			new Promise((resolve, reject) => {
				const { data } = config;
				const { params } = JSON.parse(data);

...
const Ecommerce = (state = INIT_STATE, { type, payload } = {}) => {
	switch (type) {
		...
		case UPDATE_ORDER_SUCCESS:
			return {
				...
				orders: state.orders.map((order) =>
					order.id.toString() === payload.id.toString()
						? { order, ...payload }
						: order
				),
			};
		...
		case DELETE_ORDER_SUCCESS:
			return {
				...
				orders: state.orders.filter(
					(order) => order.id.toString() !== payload.toString()
				),
			};
		...
		case UPDATE_CUSTOMER_SUCCESS:
			return {
				...
				customers: state.customers.map((customer) =>
					customer.id.toString() === payload.id.toString()
						? { customer, ...payload }
						: customer
				),
			};
		...
		case DELETE_CUSTOMER_SUCCESS:
			return {
				...
				customers: state.customers.filter(
					(customer) => customer.id.toString() !== payload.toString()
				),
			};

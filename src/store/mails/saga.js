...
function* onSetFolderOnSelectedMails({
	...
	try {
		...
		try {
			const newresponse = yield call(getMailsLists, parseInt(activeTab, 10));
		...
		try {
			const res = yield call(getselectedmails, null);
			yield put(getSelectedMailsSuccess(res));
		} catch (error) {
			yield put(getSelectedMailsFail(error));
		}
	} catch (error) {
...
function* onUpdateMail({ payload: mail }) {
	try {
		const resp = yield call(updateMail, mail);
		yield put(updateMailSuccess(resp));
	} catch (error) {
